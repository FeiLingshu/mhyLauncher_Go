...
namespace MHYLAUNCHER_GO.Obsolete
{
    ...
    /// 实现WMI互操作的帮助类
    /// </summary>
    [Obsolete("WMI组件在运行一段时间后，会由于未知原因短暂失灵一次，导致程序功能出现问题，故停止使用此类", true)]
    public class WMI
    {
        /// <summary>
        /// WMI事件响应函数实例
        /// </summary>
        public ManagementEventWatcher eventWatcher = null;
        ...
        public WMI(string name, string path, string[] args, Action<Process, string[]> _void)
        {
            this.name = name;
        ...
        private void StartWMI()
        {
            ...
            WqlEventQuery regQuery = new WqlEventQuery("__InstanceCreationEvent",
                ...
                + $@" AND TargetInstance.ExecutablePath = '{path}'"
                + $@" AND TargetInstance.Description != '0x{random.Next(0, int.MaxValue):X8}'");
            ...
            wmi_thread = new Thread(() => // 启动WMI查询循环线程
            {
                do // WMI查询循环
                {
                    ...
                    catch (ManagementException) // 捕获WMI超时信号
                    {
                        ...
                        {
                            Debug.Print("--------\n{0}ManagementException happened\n+ Targate: {1}",
                                $"[{DateTime.Now:HH:mm:ss}]",
                                eventWatcher.Query.QueryString.Replace(" AND ", "\n"));
                        }
                    ...
                    if (Debugger.IsAttached) // 调试模式下，输出WMI响应信息
                    {
                        Debug.Print("--------\n{0}WMI has been triggered",
                            $"[{DateTime.Now:HH:mm:ss}]");
                    }
                    eventWatcher.Stop();
                    eventWatcher.Dispose();
                    WqlEventQuery _regQuery = new WqlEventQuery("__InstanceCreationEvent",
                        TimeSpan.FromSeconds(1),
                        "TargetInstance ISA 'Win32_Process'"
                        + $@" AND TargetInstance.Name = '{name}'"
                        + $@" AND TargetInstance.ExecutablePath = '{path}'"
                        + $@" AND TargetInstance.Description != '0x{random.Next(0, int.MaxValue):X8}'");
                    eventWatcher = new ManagementEventWatcher(_regQuery);
                    eventWatcher.Options.Timeout = TimeSpan.FromSeconds(5);
                    ...
                    {
                        Debug.Print("--------\n{0}Process has been created\n+ Name: {1}\n+ Path: {2}\n+ PID: {3}",
                            $"[{DateTime.Now:HH:mm:ss}]",
                            ((ManagementBaseObject)e["TargetInstance"])["Name"],
                            ((ManagementBaseObject)e["TargetInstance"])["ExecutablePath"], pid.ToString("X8"));
                    }
        ...
        /// 关闭WMI事件
        /// </summary>
        public WMIhelper_Exception EndWMI(bool is_exception = false)
        {
            if (eventWatcher == null) return null;
            ...
            if (!blockover)
            {
                if (is_exception)
                {
                    return new WMIhelper_Exception("WMI组件无法正常卸载，指示代码块中可能存在阻塞，程序源代码中可能存在严重错误。");
                }
                else
                {
        ...
        /// </summary>
        public class WMIhelper_Exception : Win32Exception
        {
            public WMIhelper_Exception() : base() { }
            public WMIhelper_Exception(int error) : base(error) { }
            public WMIhelper_Exception(int error, string message) : base(error, message) { }
            public WMIhelper_Exception(string message) : base(message) { }
            public WMIhelper_Exception(string message, Exception innerException) : base(message, innerException) { }
            public WMIhelper_Exception(SerializationInfo info, StreamingContext context) : base(info, context) { }
        }
    }
}
