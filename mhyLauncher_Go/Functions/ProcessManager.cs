...
namespace MHYLAUNCHER_GO.Functions
{
    ...
    public class ProcessManager
    {
        ...
        {
            bool version = WQL.Test(infos.paths);
            BeginEventHook();
            ProcessManagerloop = new Thread(() =>
            {
                ...
                do // 监视游戏进程的启动操作
                {
                    if (games.Contains(null))
                    {
                        if (version)
                        {
                            List<int> wqlindex = new List<int>(infos.paths.Length);
                            List<string> wqllist = new List<string>(infos.paths.Length);
                            int counter = 0;
                            for (int i_path = 0; i_path < infos.paths.Length; i_path++)
                            {
                                ...
                                wqlindex.Add(i_path);
                                wqllist.Add(infos.paths[i_path]);
                                counter++;
                            }
                            if (counter > 0)
                            {
                                var gameids = WQL.GetPidsByNamesCim(wqllist);
                                for (int wqlr = 0; wqlr < counter; wqlr++)
                                {
                                    if (gameids.TryGetValue(wqllist[wqlr], out var pids))
                                    {
                                        if (pids.Count == 1)
                                        {
                                            try { games[wqlindex[wqlr]] = Process.GetProcessById((int)pids[0]); }
                                            catch (Exception) { }
                                        }
                                        else
                                        {
                                            ...
                                            foreach (var p in pids)
                                            {
                                                try
                                                {
                                                    using (var _p = Process.GetProcessById((int)p))
                                                    {
                                                        if (_p.StartTime > time)
                                                        {
                                                            pid = p;
                                                            time = _p.StartTime;
                                                        }
                                            ...
                                            if (pid > 0)
                                            {
                                                try { games[wqlindex[wqlr]] = Process.GetProcessById((int)pid); }
                                                catch (Exception) { }
                                            }
                                        }
                                    }
