...
namespace MHYLAUNCHER_GO.Functions
{
    ...
    public class Win32
    {
        ...
        [DllImport("shcore.dll")]
        public static extern int GetDpiForWindow(IntPtr hwnd, out uint dpiX, out uint dpiY);
    ...
    public static class TraceExtensions
    {
        ...
        [Obsolete("重载已过时，请使用其他重载", false)]
        public static void Print(string message, ConsoleColor fcolor)
        {
            ...
            if (Trace.Listeners.OfType<TraceListenerEx>().Any())
            {
                ...
                foreach (var line in data.message[3].Split('\n'))
                {
                    ...
                    if (linepart[0].Trim().Length == 0)
                    {
                        ...
                        Trace.Write(" · ");
                        Console.ForegroundColor = fcolor;
                        Trace.Write(linepart[1]);
                    }
                    else
                    {
                        Trace.Write($"{linepart[0]}{linepart[1]}");
                    }
                }
                Console.ForegroundColor = cc;
                Console.Out.Flush();
    ...
    public static class BugFix
    {
        ...
        public static void RefreshConsoleHandle(bool reset)
        {
            try
            {
                ...
                if (reset && is_Redirected)
                {
                    ...
                    IntPtr consoleError = CreateFile("CONERR$", GENERIC_READ | GENERIC_WRITE,
                        FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero);
                    IntPtr consoleIn = CreateFile("CONIN$", GENERIC_READ | GENERIC_WRITE,
                        FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero);
                    hwnds[0] = consoleOut;
                    hwnds[1] = consoleIn;
                    hwnds[2] = consoleError;
                    // 重定向win32流
                ...
                else
                {
                    ...
                    // 重定向.Net流
                    Console.SetOut(new StreamWriter(Console.OpenStandardOutput(), utf8) { AutoFlush = false });
                    Console.SetIn(new StreamReader(Console.OpenStandardInput(), utf8));
                    Console.SetError(new StreamWriter(Console.OpenStandardError(), utf8) { AutoFlush = false });
                }
            }
            catch (Exception)
            {
        ...
        /// </summary>
        private static void Pre(bool experiment = false)
        {
            IntPtr stdOutHandle = GetStdHandle(STD_OUTPUT_HANDLE);
            IntPtr stdErrorHandle = GetStdHandle(STD_ERROR_HANDLE);
            IntPtr stdInputHandle = GetStdHandle(STD_INPUT_HANDLE);
            bool _out = (stdOutHandle == IntPtr.Zero || stdOutHandle == INVALID_HANDLE_VALUE);
            bool _error = (stdErrorHandle == IntPtr.Zero || stdErrorHandle == INVALID_HANDLE_VALUE);
            bool _in = (stdInputHandle == IntPtr.Zero || stdInputHandle == INVALID_HANDLE_VALUE);
            if (experiment)
            {
                try
                {
                    if (!_out) CloseHandle(stdOutHandle);
                    if (!_error) CloseHandle(stdErrorHandle);
                    if (!_in) CloseHandle(stdInputHandle);
                }
                catch (Exception) { }
            }
            is_Redirected = !(_out & _error & _in);
        }
        ...
        /// 保存新句柄信息
        /// </summary>
        public static readonly IntPtr[] hwnds = new IntPtr[3];
