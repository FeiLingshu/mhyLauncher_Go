...
namespace mhyLauncher_Go
{
    ...
    internal static class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            // 执行效能模式
            if (File.Exists($"{Environment.CurrentDirectory}\\EfficiencyMode.dll"))
            {
                try
                {
                    Assembly assembly = Assembly.LoadFile($"{Environment.CurrentDirectory}\\EfficiencyMode.dll");
                    Type type_hash = assembly.GetType("EfficiencyMode.Hash");
                    Type type_main = assembly.GetType("EfficiencyMode.Main");
                    object obj_hash = Activator.CreateInstance(type_hash);
                    object obj_main = Activator.CreateInstance(type_main);
                    MethodInfo method_hash = type_hash.GetMethod("SHA512HASH");
                    MethodInfo method_main = type_main.GetMethod("CheckVersion");
                    MethodInfo method = type_main.GetMethod("Set");
                    byte[] hash = (byte[])method_hash.Invoke(obj_hash, new object[1] { $"{Environment.CurrentDirectory}\\EfficiencyMode.dll" });
                    string standard = "4919618354116108923218546227224183231674722463232271981623729217124841289118843682171161482013963239230247168487112456616525842258242262494146242161119127115223183";
                    if (string.Join("", hash) != standard) throw new ArithmeticException("文件EfficiencyMode.dll的SHA512校验未通过，指示程序文件可能遭到篡改。");
                    if ((bool)method_main.Invoke(obj_main, null))
                    {
                        bool result = (bool)method.Invoke(obj_main, new object[1] { Process.GetCurrentProcess().Handle });
                    }
                }
                catch (Exception excp)
                {
                    if (excp.GetType() == typeof(ArithmeticException)
                        && excp.Message == "文件EfficiencyMode.dll的SHA512校验未通过，指示程序文件可能遭到篡改。")
                    {
                        throw excp;
                    }
                }
            ...
            if (!INI.Skip_YuanShen)
                pt_YuanShen = new ProcessTool(new ProcessTool.Conditions("YuanShen", INI.YuanShenPath, args, MainFunction_A));
            if (!INI.Skip_StarRail)
                pt_StarRail = new ProcessTool(new ProcessTool.Conditions("StarRail", INI.StarRailPath, args, MainFunction_B));
            #region 暂时弃用的参数传递
            if (false)
            {
            ...
            if (!INI.Skip_YuanShen)
            {
                pt_YuanShen.EndProcessTool();
            }
            if (!INI.Skip_StarRail)
            {
                pt_StarRail.EndProcessTool();
            } // 释放相关资源
        }
        ...
        /// 保存原神WMI操作类实例
        /// </summary>
        private static ProcessTool pt_YuanShen = null;

        /// <summary>
        /// 保存崩坏·星穹铁道WMI操作类实例
        /// </summary>
        private static ProcessTool pt_StarRail = null;
        ...
        internal static void MainFunction_A(Process game, string[] args)
        {
            ...
            if (!report) return;
            int heightoffset = (window_r.ToRectangle().Width - client_r.ToRectangle().Width) / 2;
            ...
            if (args.Length == 1)
            {
                ...
                string[] sizeargs = args[0].ToLower().Split('x');
                if (sizeargs.Length == 2)
                {
                    if (!int.TryParse(sizeargs[0], out int width)) return;
                    if (!int.TryParse(sizeargs[1], out int height)) return;
                    targatesize = new Size(width, height);
                    iscustomsize = true;
                }
                ...
                Size workingareasize = main_s.WorkingArea.Size;
                Size windowsize = Size.Empty;
                if (iscustomsize)
                {
                    int maxwidth = workingareasize.Width - size_e.Width;
                    int maxheight = workingareasize.Height - size_e.Height - heightoffset;
                ...
                Point lction_p = Point.Empty; // 计算并应用新的窗口坐标
                if (iscustomsize)
                {
                    lction_p = main_s.WorkingArea.Location;
                    int[] ints = new int[2] { windowsize.Width, windowsize.Height };
                    lction_p.Offset(new Point()
                    {
                        X = (workingareasize.Width - windowsize.Width - size_e.Width) / 2,
                        Y = workingareasize.Height - heightoffset - windowsize.Height - size_e.Height - (int)((workingareasize.Height - heightoffset - windowsize.Height - size_e.Height) * ((double)ints.Min() / ints.Sum())) + heightoffset
                    });
                }
                else
                {
                    lction_p = main_s.Bounds.Location;
                    lction_p.Offset(new Point()
                    {
                ...
                bool SizeMove() // 调整窗口属性
                {
                    ...
                    bool location = checkout.ToRectangle().Location == rectangle.Location;
                    bool size = checkout.ToRectangle().Size == rectangle.Size;
                    switch (location && size)
                    {
                        ...
                        case false:
                            bool final_in = SetWindowPos(game.MainWindowHandle, IntPtr.Zero,
                                ...
                                SWP_DEFERERASE | SWP_NOCOPYBITS | SWP_NOZORDER);
                            if (!final_in)
                            {
                                throw new Win32Exception(Marshal.GetLastWin32Error(),
                                    $"调整窗口属性过程中出现Win32异常({Marshal.GetLastWin32Error().ToString().PadLeft(4, '0')})。");
                            };
                            long style = GetWindowLong(game.MainWindowHandle, -16);
                            SetWindowLong(game.MainWindowHandle, -16, style & ~0x00C00000L/*WS_CAPTION*/);
                            SetWindowLong(game.MainWindowHandle, -16, style);
                            SendMessage(game.MainWindowHandle, 0x0005/*WM_SIZE*/, IntPtr.Zero, (IntPtr)(windowsize.Height << 16 | windowsize.Width));
                            return true;
                    }
                    return false;
                }
                bool ChangeTitle(string str) // 修改程序标题
                {
                    ...
                    if (title.ToString() != $"原神{str}")
                    {
                        string titlestr = $"原神{str}";
                        SendMessage(game.MainWindowHandle, WM_SETTEXT, IntPtr.Zero, titlestr);
                        return true;
                    }
                    return false;
                }
                void GotoChecking() // 修改程序标题，向用户指示主要操作
                {
                    ...
                    if (time < 60000)
                    {
                        ...
                        {
                            SizeMove();
                            ChangeTitle(" | mhyLauncher_Go: 正在执行预加载 ...");
                            timecount++;
                            if (timecount == 20) break;
                            timer.WaitOne(1000);
                        } while (Thread.CurrentThread.IsAlive && !game.HasExited);
                    }
                    else
                    {
                        SizeMove();
                        ChangeTitle(" | mhyLauncher_Go: 已加载 | F11: 强制更新窗口 | F12: 切换位置锁定");
                    }
                }
        ...
        internal static void MainFunction_B(Process game, string[] args)
        {
            ...
            if (args.Length == 1)
            {
                ...
                bool ChangeTitle(string str) // 修改程序标题
                {
                    StringBuilder title = new StringBuilder(GetWindowTextLength(game.MainWindowHandle) + 1);
                    GetWindowText(game.MainWindowHandle, title, title.Capacity);
                    if (title.ToString() != $"崩坏：星穹铁道{str}")
                    {
                        string titlestr = $"崩坏：星穹铁道{str}";
                        SendMessage(game.MainWindowHandle, WM_SETTEXT, IntPtr.Zero, titlestr);
                        return true;
                    }
                    return false;
        ...
        internal static void Exception_throw(Exception e)
        {
            ...
            if (!INI.Skip_YuanShen && pt_YuanShen != null)
            {
                pt_YuanShen.EndProcessTool();
            }
            if (!INI.Skip_StarRail && pt_StarRail != null)
            {
                pt_StarRail.EndProcessTool();
            }
            // 播放提示音
