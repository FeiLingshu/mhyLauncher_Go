...
namespace mhyLauncher_Go
{
    ...
    internal static class Program
    {
        ...
        internal static void Exception_throw(Exception e)
        {
            ...
            WMIhelper_Exception wmi_YuanShen_return = null;
            Thread stopYuanShen = new Thread(() =>
            {
                if (!INI.Skip_YuanShen)
                {
                    wmi_YuanShen_return = wmi_YuanShen.EndWMI(true);
                }
                Stop_YuanShen = true;
                return;
            })
            { IsBackground = true };
            WMIhelper_Exception wmi_StarRail_return = null;
            Thread stopStarRail = new Thread(() =>
            {
                if (!INI.Skip_StarRail)
                {
                    wmi_StarRail_return = wmi_StarRail.EndWMI(true);
                }
                Stop_StarRail = true;
                return;
            })
            ...
            GC.Collect();
            string WMIhelper_Exceptions = string.Empty;
            if (wmi_YuanShen_return != null || wmi_StarRail_return != null)
            {
                WMIhelper_Exceptions = $"\n根命名空间:{e.Source}"
                    + $"\n方法体:{e.TargetSite}"
                    + $"\n详细信息:{e.GetType().Name}\n{e.Message}"
                    + $"{(Regex.IsMatch(e.Message, @"\n\z") ? string.Empty : "\n")}"
                    + $"\n位置:\n{e.StackTrace}";
