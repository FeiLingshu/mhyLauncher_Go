    ...
    internal static class Program
    {
            ...
            if (args.Length == 1)
            {
                ...
                bool SizeMove() // 调整窗口属性
                {
                    ...
                    if (checkout.ToRectangle().Location != rectangle.Location || checkout.ToRectangle().Size != rectangle.Size)
                    {
                        ...
                        //ShowWindow(game.MainWindowHandle, SW_RESTORE);
        ...
        internal static void MainFunction_B(Process game, string[] args)
        {
            ...
            if (args.Length == 1)
            {
                ...
                if (iscustomsize ? (window_r.ToRectangle().Size != windowsize) : (client_r.ToRectangle().Size != targatesize)) sizechangelimit = 1; // IMP:由于游戏会在启动后多次检测自身分辨率，故需提前判断需要执行多少次循环
                SizeMove();
                ChangeTitle();
                bool movestate = true; // 额外附加操作，冻结窗口位置
                ChangeMenu(false);
