...
<br></br>
# __游戏窗口检测逻辑更新说明__
> __由于米哈游相关游戏的窗口管理机制尚不明确，且在版本更新后经常性出现变动，导致原有的预定义的游戏窗口检测逻辑经常出现预料之外的问题，故完全重新设计了游戏窗口检测逻辑__
- [x] 阶段1：程序会在捕获到游戏进程后判断游戏已经运行的时间
- [x] 阶段2：如果时间`＜1分钟`，则开始每秒检测一次游戏窗口相关数据并做出对应更改&#42;¹，持续20秒，如果时间`＞1分钟`，则直接进入下一阶段
- [x] 阶段3：加载快捷键响应模块，开始响应用户操作，相关操作线程进入后台静默运行&#42;²  
- [ ] _`*¹：该操作进行时，窗口标题会显示"正在执行预加载 ..."`_
- [ ] _`*²：该操作进行时，窗口标题会显示相关快捷键操作的说明`_
- [ ] 程序运行时会自动处理游戏进程结束的情况，即使是在阶段2进行时，无需担心在执行预加载过程中退出游戏会产生异常
<br></br>
<br></br>
# __mhyLauncher_Go__
> ### __程序介绍__
...
参数3：2560x1440【执行窗口化宽屏适配模式，该例子为标准2K分辨率，可以替换为任意分辨率，注意，参数中字符'x'为小写英文字母"艾克斯"，该参数不含括号】
  |_ 该参数的程序支持具有以下特点
+ 具有自动纠错功能，如果输入的数值超过显示器（除任务栏以外区域）的大小，则会自动将超过限制的值设置为可用的最大值
+ 自动识别任务栏窗口，游戏窗口经调整后不会遮挡任务栏窗口
- 由于适配Windows11的需要，在Windows11系统下，游戏窗口在调整为最大可用值后四周仍会有一定间距（8px），在非Windows11系统下，相同状态时，游戏窗口上边缘可能存在极小空隙（2px）
- 由于Windows系统某些限制，在多显示器情况下，无法自动调整窗口所在的显示器，即游戏窗口可调整的最大范围为：游戏以原始方式启动时窗口所在的显示器（一般为Windows设置中的主显示器）
- 无法保证程序运行过程中与游戏窗口间的兼容性
_____________
...
```
- [x] 快捷方式[目标]栏修改举例：假设[目标]栏中原始路径为 `path` ，要填入的命令行参数为 `(a)` ，则最终[目标]栏中的字符串格式应为 `"path" (a)` ，如果使用 `参数3` 则一定不要需添加括号
...
- [x] __使用[配置模式](#%E9%85%8D%E7%BD%AE%E6%96%87%E4%BB%B6-)启动程序，并在配置窗口中点击底部 "启动 Trace 组件" 按钮__
